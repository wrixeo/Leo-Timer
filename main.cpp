...
void sound()
{
    ...
    if(music != "0")
    {
        for(int i = 0; i < music.length(); i++)
        {
            switch(music[i])
            {
                case '1': Beep(C4, duration);
                case '2': Beep(D4, duration);
                case '3': Beep(E4, duration);
                case '4': Beep(F4, duration);
                case '5': Beep(G4, duration);
                case '6': Beep(A4, duration);
                case '7': Beep(B4, duration);
            }
        }
    }
    else if(music == "0" && path != "0")
    {
        string str = "start ";
        str += path;
...
int main()
{
    while(a != "Y")
    {
        if(access("timer.psdt", F_OK))
        {
            ...
            if(outfile.is_open()) cout << "0" << endl << "clock.mp3";
        }
        ...
        else if(choose_mode == "5")
        {
            ...
            cout << "闹铃设置" << endl;
            cout << "闹铃自定义音符：(输入'x'跳过此设置，或使此设置无效。)" << endl;
            cout << "说明：使用数字代替大写字母音符。如 1=C，2=D，3=E，以此类推。" << endl;
            cout << ">>>";
            string music;
            cin.ignore();
            getline(cin, music);
